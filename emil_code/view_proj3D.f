...
c put source and detector points in ellipsoid frame
     ...
     -  ay**2*(az**2*(xbp - xsp)**2 + ax**2*(zbp - zsp)**2))**2
      ...
      subroutine matproj_linear(sinomat,
     +       projmats,
...
Cf2py intent(in) ny
Cf2py intent(in) nz
cf2py intent(in) projmats
cf2py depend(ns) projmats
Cf2py intent(out) sinomat
...
cf2py depend(nz) smat
      ...
      Do ip=1,ns
         ...
         Do kpi = 1,nz
            ...
            w21 = w2u*w1v
            w22 = w2u*w2v
            if ((iu.ge.1).and.(iu.le.nu).and.
     +          (iv.ge.1).and.(iv.le.nv)) then
               sinomat(ip,iu,iv) =
     +            sinomat(ip,iu,iv)+w11*smat(ipi,jpi,kpi)*dx*dy*dz
            endif
            if (((iu+1).ge.1).and.((iu+1).le.nu).and.
     +          (iv.ge.1).and.(iv.le.nv)) then
               sinomat(ip,iu+1,iv) =
     +            sinomat(ip,iu+1,iv)+w21*smat(ipi,jpi,kpi)*dx*dy*dz
            endif
            if ((iu.ge.1).and.(iu.le.nu).and.
     +          (iv+1.ge.1).and.(iv+1.le.nv)) then
               sinomat(ip,iu,iv+1) =
     +            sinomat(ip,iu,iv+1)+w12*smat(ipi,jpi,kpi)*dx*dy*dz
            endif
            if (((iu+1).ge.1).and.((iu+1).le.nu).and.
     +          (iv+1.ge.1).and.(iv+1.le.nv)) then
               sinomat(ip,iu+1,iv+1) =
     +            sinomat(ip,iu+1,iv+1)+w22*smat(ipi,jpi,kpi)*dx*dy*dz
            endif
         end do
         end do
         end do
...
c scan through z direction
 ...
 10   continue 
     ...
     +                                      onemat(ip,jp,kp)
      ...
      subroutine backsum(sinomat,indsino,
     +       ns,nu,nv,ds,du,dv,u0,v0,
     +       nps,npu,npv,
     +       coeffs)
...
cf2py depend(nu) indsino
cf2py depend(nv) indsino
Cf2py intent(in) coeffs
cf2py depend(nps) coeffs
cf2py depend(npu) coeffs
cf2py depend(npv) coeffs
      ...
      if (indsino(ip,jp,kp).eq.1) then
         ...
         do kpp = 1,npv
            t3= v**(kpp-1.)
            sinomat(ip,jp,kp) = sinomat(ip,jp,kp) +
     +         coeffs(ipp,jpp,kpp)*t1*t2*t3
         end do
         end do
         end do
      end if
      ...
      subroutine backsumadj(sinomat,indsino,
     +       ns,nu,nv,ds,du,dv,u0,v0,
     +       nps,npu,npv,
     +       coeffs)
...
cf2py depend(npv) coeffs
      ...
      if (indsino(ip,jp,kp).eq.1) then
         ...
         t3= v**(kpp-1.)
          ...
          coeffs(ipp,jpp,kpp) = coeffs(ipp,jpp,kpp) +
     +         sinoval*t1*t2*t3
         end do
         end do
         end do
